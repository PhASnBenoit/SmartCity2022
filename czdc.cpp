...
void CZdc::init()
{
    ...
    setAddrPlaces(cfg._addrPlaces.toInt(nullptr,16));
...
void CZdc::setAddrPlaces(int addrP)
{
    lock();
    _adrZdc->places.addr = addrP;
    unlock();
}
...
int CZdc::getAddrPlaces()
{
    int addr;
    lock();
    addr = _adrZdc->places.addr;
    unlock();
    return addr;
}
...
quint8 CZdc::getPlaces()
{
    lock();
        quint8 places = _adrZdc->places.places;
    unlock();
    return places;
}

void CZdc::setPlaces(quint8 places)
{
    lock();
        _adrZdc->places.places = places;
    unlock();
}
...
{
    lock();
        if (_adrZdc->intersection.boutonPieton1 < 128)
            _adrZdc->intersection.boutonPieton1 = (bp!=0?(bp|128):0);
    unlock();
}
...
uint8_t CZdc::getBoutonPietonVoie1()
{
    ...
    lock();
        _adrZdc->intersection.boutonPieton1 &= (~128);
...
{
    // modif pour mémoriser tant que pas lu par le client
    // on utilise le bit de poids 128
    lock();    
        if (_adrZdc->intersection.boutonPieton2 < 128)
            _adrZdc->intersection.boutonPieton2 = (bp!=0?(bp|128):0);
    unlock();
}
...
uint8_t CZdc::getBoutonPietonVoie2()
{
    uint8_t boutonPieton;
    lock();
        _adrZdc->intersection.boutonPieton2 &= (~128);
